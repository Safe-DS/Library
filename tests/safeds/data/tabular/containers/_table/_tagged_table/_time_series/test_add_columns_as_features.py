...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            Table.from_columns([Column("f2", [4, 5]), Column("f3", [6, 7])]),
            Table({"time":[0,1],"f1": [1, 2], "target": [2, 3], "other": [0, -1], "f2": [4, 5], "f3": [6, 7]}).time_columns(
                target_name="target",
                time_name = "time",
                feature_names=["f1", "f2", "f3"],
            ),
...
@pytest.mark.parametrize(
    ...
    [
        (
            TimeSeries({"time": [0,1,2],"A": [1, 2, 3], "B": [4, 5, 6]}, target_name="B",time_name = "time", feature_names=["A"]),
