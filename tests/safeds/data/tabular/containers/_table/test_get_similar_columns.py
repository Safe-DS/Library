...
@pytest.mark.parametrize(
    ...
    [
        (Table({"column1": ["col1_1"], "col2": ["col2_1"], "cilumn2": ["cil2_1"]}), "col1", ["col2"]),
        (Table({"column1": ["col1_1"], "col2": ["col2_1"], "cilumn2": ["cil2_1"]}), "clumn1", ["column1", "column3"]),
    ],
...
def test_should_warn_if_similar_column_name(table1: Table, column_name: str, expected: list[str]) -> None:
    with pytest.warns(
        UserWarning,
        match=f"The Column {column_name} is unknown.\n Did you mean one of these: {expected}?",
    ):
