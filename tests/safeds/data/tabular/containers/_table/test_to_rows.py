...
@pytest.mark.parametrize(
    ...
    [
        (Table([[1, 4, "d"], [2, 5, "e"], [3, 6, "f"]],
         ...
         [
             Row._from_pandas_dataframe(pd.DataFrame({"A": [1], "B": [4], "D": ["d"]}),
                                        Schema({"A": Integer(), "B": Integer(), "D": String()})),
             Row._from_pandas_dataframe(pd.DataFrame({"A": [2], "B": [5], "D": ["e"]}),
                                        Schema({"A": Integer(), "B": Integer(), "D": String()})),
             Row._from_pandas_dataframe(pd.DataFrame({"A": [3], "B": [6], "D": ["f"]}),
                                        Schema({"A": Integer(), "B": Integer(), "D": String()})),
         ]),
    ],
    ids=[""]
)
def test_should_return_list_of_rows(table: Table, rows_expected: list) -> None:
