...
class TestImageDatasetInit:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (_MultiSizeImageList(), Table(), ValueError, r"The given input ImageList contains images of different sizes."),
            (_EmptyImageList(), Table(), ValueError, r"The given input ImageList contains no images."),
            (ImageList.from_files(resolve_resource_path([plane_png_path, plane_png_path])), ImageList.from_files(resolve_resource_path([plane_png_path, white_square_png_path])), ValueError, r"The given output ImageList contains images of different sizes."),
            (ImageList.from_files(resolve_resource_path(plane_png_path)), _EmptyImageList(), OutputLengthMismatchError, r"The length of the output container differs"),
            (ImageList.from_files(resolve_resource_path(plane_png_path)), Table(), OutputLengthMismatchError, r"The length of the output container differs"),
            (ImageList.from_files(resolve_resource_path(plane_png_path)), Column("column", [1, 2]), OutputLengthMismatchError, r"The length of the output container differs"),
            (ImageList.from_files(resolve_resource_path(plane_png_path)), ImageList.from_files(resolve_resource_path([plane_png_path, plane_png_path])), OutputLengthMismatchError, r"The length of the output container differs"),
            (ImageList.from_files(resolve_resource_path(plane_png_path)), Table({"a": ["1"]}), NonNumericColumnError, r"Tried to do a numerical operation on one or multiple non-numerical columns: \nColumns \['a'\] are not numerical."),
            (ImageList.from_files(resolve_resource_path(plane_png_path)), Table({"a": [2]}), ValueError, r"Columns \['a'\] have values outside of the interval \[0, 1\]."),
...
class TestTableAsTensor:
    ...
    def test_should_raise_if_not_one_hot_encoded(self) -> None:
        with pytest.raises(ValueError, match=r"The given table is not correctly one hot encoded as it contains rows that have a sum not equal to 1."):
...
class TestColumnAsTensor:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (torch.randn(10), OneHotEncoder(), ValueError, r"Tensor has an invalid amount of dimensions. Needed 2 dimensions but got 1."),
            (torch.randn(10, 10, 10), OneHotEncoder(), ValueError, r"Tensor has an invalid amount of dimensions. Needed 2 dimensions but got 3."),
            (torch.randn(10, 10), OneHotEncoder(), TransformerNotFittedError, r""),
            (torch.randn(10, 10), OneHotEncoder().fit(Table({"b": ["a", "b", "c"]}), None), ValueError, r"Tensor and one_hot_encoder have different amounts of classes \(10!=3\)."),
        ]
    )
    def test_should_raise_from_tensor(self, tensor: Tensor, one_hot_encoder: OneHotEncoder, error: Type[Exception], error_msg: str) -> None:
        with pytest.raises(error, match=error_msg):
