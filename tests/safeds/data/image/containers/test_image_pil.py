...
class TestFromJpegFile:
    ...
    def test_should_load_jpeg_file(self, path: str | Path) -> None:
        ImagePil.from_jpeg_file(resolve_resource_path(path))
...
class TestFromPngFile:
    ...
    def test_should_load_png_file(self, path: str | Path) -> None:
        ImagePil.from_png_file(resolve_resource_path(path))
...
class TestFormat:
    @pytest.mark.parametrize(
        ("image", "format_"),
        [
            (ImagePil.from_jpeg_file(resolve_resource_path("image/white_square.jpg")), ImageFormat.JPEG),
            (ImagePil.from_png_file(resolve_resource_path("image/white_square.png")), ImageFormat.PNG),
        ],
        ids=["jpg", "png"],
    )
    def test_should_return_correct_format(self, image: ImagePil, format_: ImageFormat) -> None:
...
class TestProperties:
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_jpeg_file(resolve_resource_path("image/white_square.jpg")),
                    1,
                    1,
            ),
            (
                    ImagePil.from_png_file(resolve_resource_path("image/snapshot_boxplot.png")),
                    640,
                    480,
            ),
        ],
    ...
    def test_should_return_image_properties(self, image: ImagePil, width: int, height: int) -> None:
...
class TestToJpegFile:
    ...
    def test_should_save_jpeg_file_by_str(self, path: str) -> None:
        image = ImagePil.from_jpeg_file(resolve_resource_path(path))
        ...
        with Path(tmp_file.name).open("rb") as tmp_read_file:
            image_read_back = ImagePil.from_jpeg_file(tmp_read_file.name)
...
class TestToPngFile:
    ...
    def test_should_save_png_file_by_str(self, path: str) -> None:
        image = ImagePil.from_png_file(resolve_resource_path(path))
        ...
        with Path(tmp_file.name).open("rb") as tmp_read_file:
            image_read_back = ImagePil.from_png_file(tmp_read_file.name)
...
class TestReprJpeg:
    ...
    def test_should_return_bytes_if_image_is_jpeg(self, image: ImagePil) -> None:
        assert isinstance(image._repr_jpeg_(), bytes)

    @pytest.mark.parametrize(
        "image",
        [ImagePil.from_png_file(resolve_resource_path("image/white_square.png"))],
        ids=["png"],
    )
    def test_should_return_none_if_image_is_not_jpeg(self, image: ImagePil) -> None:
...
class TestReprPng:
    ...
    def test_should_return_bytes_if_image_is_png(self, image: ImagePil) -> None:
    ...
    def test_should_return_none_if_image_is_not_png(self, image: ImagePil) -> None:
...
class TestConvertToGrayscale:
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_png_file(resolve_resource_path("image/snapshot_heatmap.png")),
                    ImagePil.from_png_file(resolve_resource_path("image/snapshot_heatmap_grayscale.png")),
            ),
        ],
        ids=["grayscale"],
    )
    def test_convert_to_grayscale(self, image: ImagePil, expected: ImagePil) -> None:
...
class TestEQ:
    def test_should_be_equal(self) -> None:
        image = ImagePil.from_png_file(resolve_resource_path("image/original.png"))
        image2 = ImagePil.from_png_file(resolve_resource_path("image/copy.png"))
        assert image == image2

    def test_should_not_be_equal(self) -> None:
        image = ImagePil.from_png_file(resolve_resource_path("image/original.png"))
        image2 = ImagePil.from_png_file(resolve_resource_path("image/white_square.png"))
        assert image != image2
...
class TestFlipVertically:
    def test_should_flip_vertically(self) -> None:
        ...
        image3 = ImagePil.from_png_file(resolve_resource_path("image/flip_vertically.png"))
        assert image != image2
...
class TestFlipHorizontally:
    def test_should_flip_horizontally(self) -> None:
        ...
        image3 = ImagePil.from_png_file(resolve_resource_path("image/flip_horizontally.png"))
        assert image != image2
        assert image2 == image3
...
class TestAdjustContrast:
    ...
    def test_should_adjust_contrast(self, factor: float) -> None:
        image = ImagePil.from_png_file(resolve_resource_path("image/contrast/to_adjust_contrast.png"))
        image2 = image.adjust_contrast(factor)
        image3 = ImagePil.from_png_file(
    ...
    def test_should_raise(self) -> None:
        image = ImagePil.from_png_file(resolve_resource_path("image/brightness/to_brighten.png"))
...
class TestBrightness:
    ...
    def test_should_adjust_brightness(self, factor: float) -> None:
        ...
        image3 = ImagePil.from_png_file(resolve_resource_path("image/brightness/brightened_by_" + str(factor) + ".png"))
        assert image != image2
        assert image2 == image3
...
class TestInvertColors:
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_png_file(resolve_resource_path("image/original.png")),
                    ImagePil.from_png_file(resolve_resource_path("image/inverted_colors_original.png")),
            ),
        ],
        ids=["invert-colors"],
    )
    def test_should_invert_colors(self, image: ImagePil, expected: ImagePil) -> None:
...
class TestColorAdjust:
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_png_file(resolve_resource_path("image/original.png")),
                    2,
                    ImagePil.from_png_file(resolve_resource_path("image/adjusted_colors/by_2.png")),
            ),
            (
                    ImagePil.from_png_file(resolve_resource_path("image/original.png")),
                    0.5,
                    ImagePil.from_png_file(resolve_resource_path("image/adjusted_colors/by_0.5.png")),
            ),
            (
                    ImagePil.from_png_file(resolve_resource_path("image/original.png")),
                    0,
                    ImagePil.from_png_file(resolve_resource_path("image/adjusted_colors/by_0.png")),
            ),
        ],
        ids=["add color", "remove color", "remove all color"],
    )
    def test_should_adjust_colors(self, image: ImagePil, factor: float, expected: ImagePil) -> None:
    ...
    def test_should_throw(self, image: ImagePil, factor: float) -> None:
    ...
    def test_should_warn(self, image: ImagePil, factor: float) -> None:
        with pytest.warns(
            UserWarning,
...
class TestAddGaussianNoise:
    @pytest.mark.parametrize(
        ...
        [
            (ImagePil.from_png_file(resolve_resource_path("image/boy.png")), 0.0),
            (ImagePil.from_png_file(resolve_resource_path("image/boy.png")), 0.7),
            (ImagePil.from_png_file(resolve_resource_path("image/boy.png")), 2.5),
        ],
        ids=["minimum noise", "some noise", "very noisy"],
    )
    def test_should_add_noise(self, image: ImagePil, standard_deviation: float) -> None:
        expected = ImagePil.from_png_file(
    ...
    @pytest.mark.parametrize(
        ("image", "standard_deviation"),
        [(ImagePil.from_png_file(resolve_resource_path("image/boy.png")), -1)],
        ids=["sigma below zero"],
    )
    def test_should_raise_standard_deviation(self, image: ImagePil, standard_deviation: float) -> None:
        with pytest.raises(
            OutOfBoundsError,
...
class TestBlur:
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_png_file(resolve_resource_path("image/boy.png")),
                    ImagePil.from_png_file(resolve_resource_path("image/blurredBoy.png")),
            ),
        ],
        ids=["blur"],
    )
    def test_should_return_blurred_image(self, image: ImagePil, expected: ImagePil) -> None:
        image = image.blur(2)
        assert image == expected
...
class TestCrop:
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_png_file(resolve_resource_path("image/white.png")),
                    ImagePil.from_png_file(resolve_resource_path("image/whiteCropped.png")),
            ),
        ],
        ids=["crop"],
    )
    def test_should_return_cropped_image(self, image: ImagePil, expected: ImagePil) -> None:
...
class TestSharpen:
    ...
    def test_should_sharpen(self, factor: float) -> None:
        image = ImagePil.from_png_file(resolve_resource_path("image/sharpen/to_sharpen.png"))
        image2 = image.sharpen(factor)
        # image2.to_png_file(resolve_resource_path("image/sharpen/sharpened_by_" + str(factor) + ".png"))
        assert image != image2
        assert image2 == ImagePil.from_png_file(
...
class TestRotate:
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_png_file(resolve_resource_path("image/snapshot_boxplot.png")),
                    ImagePil.from_png_file(resolve_resource_path("image/snapshot_boxplot_right_rotation.png")),
            ),
        ],
        ids=["rotate-clockwise"],
    )
    def test_should_return_clockwise_rotated_image(self, image: ImagePil, expected: ImagePil) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_png_file(resolve_resource_path("image/snapshot_boxplot.png")),
                    ImagePil.from_png_file(resolve_resource_path("image/snapshot_boxplot_left_rotation.png")),
            ),
        ],
        ids=["rotate-counter-clockwise"],
    )
    def test_should_return_counter_clockwise_rotated_image(self, image: ImagePil, expected: ImagePil) -> None:
...
class TestFindEdges:
    @pytest.mark.parametrize(
        ...
        [
            (
                    ImagePil.from_png_file(resolve_resource_path("image/boy.png")),
                    ImagePil.from_png_file(resolve_resource_path("image/edgyBoy.png")),
            ),
        ],
        ids=["find_edges"],
    )
    def test_should_return_edges_of_image(self, image: ImagePil, expected: ImagePil) -> None:
