...
class TestBaselineRegressor:
    ...
    def test_check_predict_return_type_and_values(self) -> None:
        ...
        assert result.get("coefficient_of_determination") is not None and result.get("coefficient_of_determination") >= float("-inf")
        assert result.get("mean_absolute_error") is not None and result.get("mean_absolute_error") <= float("inf")
        assert result.get("mean_squared_error") is not None and result.get("mean_squared_error") <= float("inf")
        assert result.get("median_absolute_deviation") is not None and result.get("median_absolute_deviation") <= float("inf")
