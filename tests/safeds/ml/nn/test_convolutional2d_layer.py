...
class TestConvolutional2DLayer:
    ...
    def test_should_raise_if_activation_function_not_set(self, conv_type: Type[Convolutional2DLayer], output_channel: int, kernel_size: int, stride: int, padding: int) -> None:
        layer = conv_type(output_channel, kernel_size, stride=stride, padding=padding)
        input_size = ImageSize(10, 20, 30, _ignore_invalid_channel=True)
        layer._set_input_size(input_size)
    ...
    def test_should_raise_if_unsupported_activation_function_is_set(self, conv_type: Type[Convolutional2DLayer], output_channel: int, kernel_size: int, stride: int, padding: int) -> None:
        layer = conv_type(output_channel, kernel_size, stride=stride, padding=padding)
        input_size = ImageSize(10, 20, 30, _ignore_invalid_channel=True)
        layer._set_input_size(input_size)
        with pytest.raises(ValueError, match=r"The activation_function 'unknown' is not supported. Please choose one of the following: \['sigmoid', 'relu', 'softmax'\]."):
    ...
    def test_should_raise_if_input_size_is_set_with_int(self, conv_type: Type[Convolutional2DLayer], output_channel: int, kernel_size: int, stride: int, padding: int) -> None:
        layer = conv_type(output_channel, kernel_size, stride=stride, padding=padding)
