...
class Model:
    ...
    def train(self, train_data: TaggedTable, epoch_size=25, batch_size=1):
        # Transform TaggedTable to Dataloader
        dataloader = train_data.into_dataloader(batch_size)

        # TODO Determine Loss Function based on use case
        loss_fn = nn.MSELoss()

        # TODO Decide whether user should be able to customize the learning rate
        optimizer = torch.optim.SGD(self._model.parameters(), lr=0.005)
